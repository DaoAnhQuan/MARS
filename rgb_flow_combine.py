...
channels = 2
checkpoint = 'models/flow_split1_epoch099.pt'
flow_model = I3DCustom(NUM_CLASS, in_channels=channels)
flow_model = flow_model.cuda()
flow_model.load_state_dict(torch.load(checkpoint))
flow_model.eval()

def combine(rgb, flow, mode='avg'):
        if mode=='avg':
                return rgb+flow
        else:
                x = torch.cat([rgb, flow], dim=0)
                return torch.max(x, dim=0).values.unsqueeze(0)
with torch.no_grad():
        end = time.time()
        for step, (rgb, flow, target) in enumerate(val_loader):
                ...
                flow_output, _, _, _ = flow_model(flow)
                output = combine(rgb_output, flow_output)
